---
title: 自訂禮賓的視覺風格 — 影片
description: 瞭解如何自訂Adobe Brand Concierge的視覺樣式，包括歡迎訊息、入門提示卡和卡片顏色。
topic: Personalization
role: User
level: Beginner
doc-type: Technical Video
duration: 234
last-substantial-update: 2026-08-25T00:00:00Z
jira: KT-22408
source-git-commit: e4579efe202448c0c4dc85b3444abac6ed2834c2
workflow-type: tm+mt
source-wordcount: '160'
ht-degree: 0%

---

# 自訂禮賓的視覺風格

您的Adobe Brand Concierge從Adobe已為您做出的視覺選擇開始，也就是從您的網站中拉取的調色盤、字型和卡片影像。 在本影片中，您將會檢閱這些選擇，並自訂歡迎訊息和入門提示卡，以更符合您的品牌。

## 這部影片是給誰看的？

* 品牌和行銷團隊
* 內容設計人員和撰稿人
* 任何微調門房外觀的人

## 視訊內容

* Brand Concierge如何從您的網站取得其入門色彩、字型和卡片影像
* 如何測試預設的桌上型電腦和行動裝置外觀
* 如何編輯歡迎訊息標題和副標題，包括文字顏色、粗細和大小
* 如何新增、移除和編輯入門提示卡 — 包括卡片文字、影像和背景顏色

>[!VIDEO](https://video.tv.adobe.com/v/3502259)

如需相關檔案，請參閱[Brand Concierge說明](../documentation/overview.md)。
